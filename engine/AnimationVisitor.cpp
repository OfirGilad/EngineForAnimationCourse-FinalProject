...
namespace cg3d
{
    ...
    void AnimationVisitor::Visit(Model* model)
    {
        ...
        if (scene->GetAnimate())
        {
            ...
            {
                if (model->name == std::string("bone 15"))
                {
                    float movement_speed = float(scene->game_manager->stats->current_movement_speed) * 0.001f;
                    //model->TranslateInSystem(system, Eigen::Vector3f(0, 0, movement_speed));

                    scene->game_manager->fabrik_logic.SetDestinationPosition(model->GetTranslation() + system * Eigen::Vector3f(0, 0, movement_speed));
                    scene->game_manager->fabrik_logic.IKFabrikMethod();
